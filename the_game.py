...
def read_game(fich):
    ...
    N = int(a[0])                  # and expecting N to be the first thing on the list xc

    # a seria uma cena do genero = [N, 0000, 8880, 0000] , para um tabuleiro de 4x4

    a = a[1::] # i removed the 1st thing which again i assume its N so that the positions are alright now

    for i in range(N): 
        for j in range(N):		# these for do not work, index out of range, same thing for the next function, the rest is alright
            tab[i][j] = a[i][j]		# they're imcomplete tho
